...
for robot in warehouse.robots:
    assert robot.robot_domain[0] <= robot.get_position()[0] <= robot.robot_domain[2] and \
      robot.robot_domain[1] <= robot.get_position()[1] <= robot.robot_domain[3], \
...
actions = np.zeros(len(warehouse.robots), dtype=np.int)
...
for robot in warehouse.robots:
    initial_positions.append(robot.get_position())
actions = np.ones(len(warehouse.robots), dtype=np.int)
warehouse.step(actions)
for robot, initial_position in zip(warehouse.robots, initial_positions):
    assert robot.get_position()[1] + 1 == initial_position[1], "action test: failed"
# action 2
